...
#
# libdwarf has historically never been linted in illumos particularly when it
# came from the tools area. We should probably investigate linting it when we do
# a serious update and can figure out if the upstream would be responsive to
# these changes (or look at eliminating it).
#
LIBDWARF = -ldwarf
lintcheck := LIBDWARF =
LIBS = $(DYNLIB) $(LINTLIB)
...
LINTCHECKFLAGS += -erroff=E_BAD_FORMAT_ARG_TYPE2
