...
i2c_device_add_req_exec(i2c_dev_add_req_t *req)
{
	...
	if (pack_buf == NULL) {
		int e = errno;
		ret = i2c_error(hdl, I2C_ERR_NO_MEM, e, "failed to allocate "
		    "%zu bytes for packed request nvlist", pack_size);
		goto out;
	}
 ...
 * all of the minors that are I2C devices. So we first gather that up and merry
 ...
 * dev info nodes that end up under the port that aren't acutally in-band
    ...
    i2c_port_t **portp, i2c_dev_info_t **infop)
{
	...
	if (!i2c_path_parse(hdl, path, root, &dn, &type)) {
		di_fini(root);
		return (false);
	}
