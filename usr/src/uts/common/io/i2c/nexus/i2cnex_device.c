...
i2c_device_config(i2c_port_t *port, i2c_dev_t *dev)
{
	 ...
	 * exist), then this will fail. We use the existence of the resultant
	 * dev_info_t as a proxy for this.
	 */
	(void) snprintf(ua, sizeof (ua), "%s@%s", dev->id_nex->in_name,
	    dev->id_nex->in_addr);
	(void) ndi_devi_config_one(port->ip_nex->in_dip, ua, &ign, NDI_CONFIG |
	    NDI_ONLINE_ATTACH);
	if (dev->id_nex->in_dip == NULL) {
		return (false);
	}
