 ...
 * any time on the bus and all I/O is initiated by the controller (generally
 * speaking). This ends up playing a lot into the overall system design. If
 * someone else tries to talk on the bus at the same time, this is considered a
 * collision and will cause an error.
 *
 * ------------
 * Key Concepts
 * ------------
 ...
 *
 *  2. When calling into an NDI operation, one is not allowed o hold a
...
/*
 * Attempt to unconfigure one of our nexi. This is a little tricky. While we
 * have called into ndi_busop_bus_unconfig, because these are often nexus nodes,
 * it may not have done anything. Therefore we cannot assume that
 * ndi_devi_free() will actually succeed or that we'll be in a state that it
 * makes sense to. As a result, if the node is still attached, we don't actually
 * do anything and return failure.
 */
static int
i2c_nex_bus_unconfig_i2cnex(i2c_nexus_t *nex)
{
	...
	VERIFY3P(nex, ==, ddi_get_parent_data(nex->in_dip));

	/*
	 * If we have a driver attached right now or there is still a reference
	 * count on this node, then we can't tear it down. The former is most
	 * common for our nexus drivers. The latter is when we have a child
	 * which is a device.
	 */
	...
	/*
	 * There are no registers or other properties to free at this time, so
	 * we just attempt to free the node. We expect that ndi_devi_free should
	 * not fail, but if it it does, we warn about it, but don't propagate
	 * that fact.
	 */
	...
	if (ndi_devi_free(nex->in_dip) != NDI_SUCCESS) {
		dev_err(nex->in_dip, CE_WARN, "failed to free dip in unconfig");
	}
