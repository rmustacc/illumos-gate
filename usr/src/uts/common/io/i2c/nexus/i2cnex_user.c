...
/*
 * Acquire exclusive access to the i2c_user_t for performing I/O. This occurs in
 * two steps. First it gets exclusive access to the i2c_user_t and then it
 * attempts to acquire the bus.
 */
static bool
...
i2c_nex_dev_add_nvl_parse(nvlist_t *nvl, i2c_error_t *err, i2c_dev_add_t *add)
{
	...
	add->da_addr.ia_type = UINT16_MAX;
	add->da_addr.ia_addr = UINT16_MAX;
	...
	if (add->da_addr.ia_type == UINT16_MAX ||
	    add->da_addr.ia_addr == UINT16_MAX ||
	    add->da_name == NULL) {
