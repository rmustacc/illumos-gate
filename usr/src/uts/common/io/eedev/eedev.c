 ...
 * doesn't give us something more specific.
 */
...
/*
 * We are called here by one or more thread that are trying to open a specific
...
restart:
	 ...
	 * for us to do. The kernel guarnatees that it won't call close() on
	...
	if ((hdl->eh_flags & EEDEV_F_BUSY) != 0) {
		hdl->eh_nwaiters++;
		while ((hdl->eh_flags & EEDEV_F_BUSY)) {
	...
	dev_info_t *pdip = hdl->eh_dip->ed_dip;
	ndi_devi_enter(pdip);
	 ...
	 * drop the lock again, and thus hae another chance of a race condition.
	 */
	if ((hdl->eh_dip->ed_flags & EEDEV_F_USABLE) == 0) {
		dev_info_t *ign;
		mutex_exit(&eedev.eedev_mutex);
		(void) ndi_devi_config_one(pdip, hdl->eh_dip->ed_ua, &ign,
...
eedev_minor_create(eedev_hdl_t *hdl)
{
	...
	if (ddi_create_minor_node(eedev.eedev_dip, hdl->eh_name, S_IFCHR,
	    hdl->eh_minor, DDI_NT_EEPROM, 0) != DDI_SUCCESS) {
		dev_err(eedev.eedev_dip, CE_WARN, "!failed to create eeprom "
		    "minor %s (%d) for %s%d", hdl->eh_name, hdl->eh_minor,
		    ddi_driver_name(hdl->eh_dip->ed_dip),
		    ddi_get_instance(hdl->eh_dip->ed_dip));
		return (false);
	}
...
eedev_create(const eedev_reg_t *reg, eedev_hdl_t **hdlp)
{
	...
	if (hdl != NULL) {
		VERIFY0(hdl->eh_flags & EEDEV_F_HELD);
	...
	/*
	 * Check to make sure that this name is unique across all devices.
	 */
	for (eedev_hdl_t *h = list_head(&eedev.eedev_list); h != NULL;
	    h = list_next(&eedev.eedev_list, h)) {
...
eedev_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
	...
	mutex_enter(&eedev.eedev_mutex);
	if (list_is_empty(&eedev.eedev_list) != 0) {
		mutex_exit(&eedev.eedev_mutex);
		return (DDI_FAILURE);
	}
